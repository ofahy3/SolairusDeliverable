...
async def generate_report(request: GenerationRequest, background_tasks: BackgroundTasks):
    ...
    return {"message": "Report generation started", "status": "processing", "session_id": session_id}
...
async def run_generation(session_id: str, test_mode: bool, focus_areas: Optional[List[str]]):
    ...
    try:
        ...
        if len(sessions) > 50:
            oldest_keys = sorted(sessions.keys(), key=lambda k: sessions[k].get("created_at", ""))[:len(sessions)-50]
