...
def test_validator():
    """Test the fact validation functionality"""
    print("=" * 60)
    print("FACT VALIDATOR TEST")
    print("=" * 60)

    validator = FactValidator()

    # Test 1: Valid AI output (claims supported by sources)
    print("\nTest 1: Valid AI Output")

    source_items = [
        IntelligenceItem(
            raw_content="US inflation 3.5%",
            processed_content="US inflation rose to 3.5% in Q4 2025, impacting business aviation costs",
            category="economic",
            relevance_score=0.9,
    ...
    ai_output_valid = "Ergo assesses that US inflation at 3.5% will drive increased operating costs for business aviation through Q4 2025."

    is_valid, unsupported = validator.validate_ai_output(ai_output_valid, source_items)
    print(f"AI Output: {ai_output_valid}")
    ...
    print(f"Validation: {'✓ PASS' if is_valid else '✗ FAIL'}")
    if unsupported:
        print(f"Unsupported claims: {', '.join(unsupported)}")

    # Test 3: Analytical text without specific claims (safe)
    print("\nTest 3: Analytical Text Without Specific Claims")

    ai_output_analytical = "Ergo assesses that geopolitical tensions in the region will continue to affect aviation operations."

    is_valid, unsupported = validator.validate_ai_output(ai_output_analytical, source_items)
    print(f"AI Output: {ai_output_analytical}")
    print(f"Validation: {'✓ PASS' if is_valid else '✗ FAIL'}")
    ...
    print("\n" + "=" * 60)
    print("✓ Fact Validator test complete")
    print("=" * 60)
