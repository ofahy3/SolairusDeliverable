...
class ErgoMindProcessor(BaseProcessor):
    ...
    def _clean_and_structure(self, text: str) -> str:
        ...
        hedging_patterns = [
            'has not identified', 'have not identified', 'no evidence of',
            'does not appear', 'not identified', 'no significant new',
            'no major new', 'unclear whether', 'insufficient data',
    ...
    async def _generate_so_what_async(self, text: str, category: str, item: Optional[IntelligenceItem] = None) -> str:
                ...
                try:
                    ...
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        future = executor.submit(
                            asyncio.run,
                            self._generate_so_what_async(text, category, item)
                        )
