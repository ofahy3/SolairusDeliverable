...
class GTAProcessor(BaseProcessor):
    ...
    def _calculate_relevance(self, intervention) -> float:
        ...
        is_aviation_relevant = any(
            keyword in " ".join(sector_strings) for keyword in aviation_adjacent
        )
    ...
    def _generate_action_items(self, intervention, sectors: List[ClientSector]) -> List[str]:
        ...
        if "sanction" in intervention_type_lower or "export" in intervention_type_lower:
            actions.append(
                "Conduct compliance review for all affected jurisdictions and update routing protocols"
            )
            actions.append(
                "Brief affected clients on travel restrictions and alternative routing options"
            )
            actions.append("Review client list for exposure to sanctioned entities or regions")

        elif "tariff" in intervention_type_lower:
            actions.append(
                "Monitor for secondary impacts on client business operations and travel demand"
            )
