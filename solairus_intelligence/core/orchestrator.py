...
from solairus_intelligence.clients.ergomind_client import ErgoMindClient, QueryResult, ErgoMindConfig
from solairus_intelligence.core.processor import IntelligenceProcessor, IntelligenceItem, ClientSector
...
class QueryOrchestrator:
    ...
    async def execute_monthly_intelligence_gathering(self) -> Dict[str, List[QueryResult]]:
        ...
        logger.info(f"Starting parallel intelligence gathering with {len(sorted_templates)} query templates")
        ...
        async with self.client:
            ...
            async def execute_template_with_semaphore(template: QueryTemplate) -> Tuple[str, List[QueryResult]]:
                """Execute a single template's queries with semaphore control"""
                async with semaphore:
                    logger.info(f"Processing category: {template.category} (Priority: {template.priority})")
                    ...
                    if main_result.success:
                        ...
                        if main_result.confidence_score > 0.6 and template.follow_ups:
                            for follow_up in template.follow_ups[:2]:  # Limit follow-ups to avoid overload
                                await asyncio.sleep(1)  # Reduced from 2s since we have semaphore control
        ...
        logger.info(f"Parallel intelligence gathering complete. Collected {sum(len(v) for v in results.values())} responses")
        return results
        
    async def process_and_filter_results(self, raw_results: Dict[str, List[QueryResult]]) -> List[IntelligenceItem]:
        """
        ...
        for item in processed_items:
            ...
            for existing in unique_items:
                ...
                if len(item.processed_content) > 100 and len(existing.processed_content) > 100:
                    if item.processed_content[:100].lower() == existing.processed_content[:100].lower():
                        is_duplicate = True
                        break
            if not is_duplicate:
        ...
        logger.info(f"Processed {len(unique_items)} unique intelligence items from {len(processed_items)} total items")
        return unique_items
        
    async def generate_focused_queries(self, focus_areas: Optional[List[str]] = None) -> List[QueryTemplate]:
        """
        ...
        for area in focus_areas:
            ...
            for template in self.query_templates:
                if area_lower in template.category.lower() or \
                   any(area_lower in q.lower() for q in [template.query] + template.follow_ups):
    ...
    def optimize_query_order(self, templates: List[QueryTemplate]) -> List[QueryTemplate]:
        ...
        for template in templates:
            if any(region in template.category for region in ['america', 'europe', 'asia', 'middle']):
    ...
    async def execute_gta_intelligence_gathering(self, days_back: int = 60) -> Dict[str, List]:
        ...
        async with GTAClient() as gta_client:
            ...
            query_tasks = {
                'sanctions_trade': gta_client.get_sanctions_and_export_controls(days=days_back, limit=20),
                'capital_controls': gta_client.get_capital_controls(days=days_back, limit=15),
                'technology_restrictions': gta_client.get_technology_restrictions(days=days_back, limit=15),
                'aviation_sector': gta_client.get_aviation_sector_interventions(days=90, limit=10),
                'harmful_interventions': gta_client.get_recent_harmful_interventions(days=days_back, limit=20)
            }
        ...
        logger.info(f"GTA intelligence gathering complete. Collected {total_interventions} interventions")
    ...
    async def process_gta_results(self, gta_results: Dict[str, List]) -> List[IntelligenceItem]:
        ...
        for category, interventions in gta_results.items():
            for intervention in interventions:
                try:
                    ...
                    logger.error(f"Failed to process GTA intervention {intervention.intervention_id}: {e}")
                    continue
        ...
        logger.info(f"Processed {len(unique_items)} unique GTA intelligence items from {len(processed_items)} total")
    ...
    async def execute_fred_data_gathering(self, days_back: int = 90) -> Dict[str, List]:
        ...
        logger.info(f"FRED data gathering complete. Collected {total_observations} economic indicators")
        ...
        for category, observations in fred_results.items():
            for observation in observations:
                try:
                    ...
                    processed_items.append(item)
                    logger.debug(f"Processed FRED {category}: {observation.series_name} = {observation.value}")
                except Exception as e:
                    logger.error(f"Failed to process FRED observation {observation.series_id}: {str(e)}")
                    continue
