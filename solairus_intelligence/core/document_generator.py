...
class DocumentGenerator:
    ...
    def __init__(self):
        ...
        # Path to logo
        self.logo_path = Path(__file__).parent.parent / "web" / "static" / "images" / "ergo_logo.png"
            ...
            if len(watch_factors) < 3:
                for row_idx in range(len(watch_factors), 3):
                    ...
                    if placeholder:
                        ...
                        data_row.cells[1].text = placeholder.get('what_to_watch', 'Monitoring in progress')
                        data_row.cells[2].text = placeholder.get('why_it_matters', 'Analysis pending')
        ...
        if self.ai_generator:
            try:
                ...
                try:
                    ...
                    # If there's a running loop, use nest_asyncio or create a new thread
    ...
    def _craft_watch_factor_statement(self, item: IntelligenceItem) -> str:
        ...
        return f"Monitor: {first_sentence} - potential implications for business aviation operations."
    ...
    def _parse_key_finding(self, finding_text: str) -> tuple:
        ...
        elif 'china' in text_lower and ('us' in text_lower or 'trade' in text_lower or 'truce' in text_lower):
            subheader = "US-China relations"
        elif 'inflation' in text_lower or 'interest rate' in text_lower or 'federal reserve' in text_lower:
            subheader = "Monetary policy & financing"
        elif 'tariff' in text_lower or 'trade' in text_lower:
            subheader = "Trade policy & tariffs"
        elif 'technology' in text_lower or 'semiconductor' in text_lower or 'export control' in text_lower:
        ...
        if len(sentences) >= 3:
            ...
            bullets = [s.strip() + ('.' if not s.strip().endswith('.') else '') for s in sentences[2:] if s.strip()]
        elif len(sentences) == 2:
            content = sentences[0] + '.'
            bullets = [sentences[1].strip() + ('.' if not sentences[1].strip().endswith('.') else '')]
        else:
            content = text
            bullets = []
        ...
        if len(bullets) < 2:
            bullets.extend(self._generate_contextual_bullets(text_lower, subheader, 2 - len(bullets)))
    ...
    def _parse_watch_factor(self, factor_text: str) -> tuple:
        ...
        if 'data center' in text_lower:
            ...
            why_it_matters = "Delays indicate fiscal constraints affecting European corporate travel budgets"
        elif 'supply chain' in text_lower:
            indicator = "Supply Chain Decoupling"
            what_to_watch = "US-China supply chain separation velocity"
            why_it_matters = "Accelerating separation drives increased site visit requirements for tech clients"
            ...
            why_it_matters = "Compliance requirements may change rapidly for international operations"
    ...
    def _add_economic_indicators_table(self, doc: Document, items: List[IntelligenceItem]):
        ...
        for item in fred_items[:5]:  # Limit to 5 indicators
            ...
            impact = item.so_what_statement if item.so_what_statement else self._generate_economic_impact(item)
    ...
    def _extract_value(self, item: IntelligenceItem) -> str:
        ...
        dollar_match = re.search(r'\$(\d+\.?\d*)\s*(billion|million|trillion)?', content, re.IGNORECASE)
        if dollar_match:
    ...
    def _craft_regional_assessment(self, region: str, primary_item: IntelligenceItem, all_items: List[IntelligenceItem]) -> str:
        ...
        if region == 'North America':
            ...
            elif 'inflation' in content_lower or 'economic' in content_lower or 'fed' in content_lower:
                return self._build_assessment_sentence(
        ...
        return self._build_assessment_sentence(
            "Developments in this region warrant continued monitoring.",
            content, primary_item
        )

    def _build_assessment_sentence(self, template: str, content: str, item: IntelligenceItem) -> str:
