...
class SolairusIntelligenceGenerator:
    """
    Main application class for generating Solairus intelligence reports
    """
    ...
    async def generate_monthly_report(
        ...
        try:
            logger.info("=" * 50)
            logger.info("Starting Solairus Intelligence Report Generation")
            ...
            if test_mode:
                ...
                gta_results = await self.orchestrator.execute_gta_intelligence_gathering(days_back=30)
                ...
                multi_source_results = await self.orchestrator.execute_multi_source_intelligence_gathering()

                # Capture source status for reporting
                source_status = multi_source_results.get('source_status', {
                    'ergomind': 'unknown',
                    'gta': 'unknown',
                    'fred': 'unknown'
                })
                ...
                ergomind_items = await self.orchestrator.process_and_filter_results(multi_source_results['ergomind'])
            ...
            logger.info(f"✓ Collected {status['queries_executed']} total responses (ErgoMind + GTA + FRED)")
            ...
            status['ergomind_count'] = len([i for i in processed_items if i.source_type == "ergomind"])
            ...
            logger.info(f"  Source breakdown: {status['ergomind_count']} ErgoMind + {status['gta_count']} GTA + {status['fred_count']} FRED")
    ...
    async def _test_intelligence_gathering(self) -> Dict:
        ...
        ][:6]  # Use 6 queries instead of 3 for more content
