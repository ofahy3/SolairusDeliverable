"""
MRO Intelligence Report Style Configuration

This module contains the editorial standards and formatting requirements
for generating monthly intelligence reports for Grainger. These rules ensure output
...
SECTION_HEADERS = {
    ...
    "examples": {
        "good": [
            "**EXECUTIVE SUMMARY**",
            "**ECONOMIC INDICATORS FOR MRO DEMAND**",
            "**Manufacturing Sector Outlook**",
            "**Supply Chain Disruption Risk**",
        ],
        "bad": [
            "EXECUTIVE SUMMARY",  # unbolded
            "**Manufacturing sector outlook**",  # sentence case
        ],
    },
}
...
BOTTOM_LINE_RULES = {
    ...
    "content_rules": [
        "Synthesize all sector themes into one cohesive statement",
        "Remove tangential topics not directly relevant to MRO demand",
        "Every topic mentioned MUST have a corresponding Key Findings section",
    ],
    "avoid": [
        "Multiple paragraphs covering unrelated topics",
        'Generic "high-risk scenario" language',
        "Topics without corresponding Key Findings sections",
    ],
    "example": (
        "**Ergo assesses that tariff escalation on Chinese imports would trigger immediate "
        "supply chain disruptions for industrial components,** forcing rapid supplier "
        "diversification in the manufacturing sector, while construction and energy "
        "operations face persistent material cost inflation from commodity market volatility "
        "and regulatory compliance overhead."
    ),
}
...
KEY_FINDINGS_RULES = {
    ...
    "bullet_examples": {
        "good": [
            "-   **Supply chain disruption.** Rerouting via alternative suppliers...",
            "-   **Material cost inflation.** Procurement should assume...",
            "-   **Regulatory compliance overhead.** Supply chain teams...",
        ],
        "bad": [
            "> • Supply chain disruption would require rerouting...",
            "- Supply chain disruption: teams should...",  # not bold
        ],
    },
}
...
PREFERRED_TERMINOLOGY = {
    "elevated cost thresholds": "higher cost thresholds",
    "ceasing supplier deliveries entirely": "causing supplier deliveries to cease entirely",
    "tariffs have failed to reduce": "prove resilient to tariffs",
    "sustaining pressure on": "maintaining pressure on",
    "hardening procurement restrictions": "tighten procurement policies",
}

AVOID_VERBOSE = [
    "Ergo assesses that... at the start of every section",
    "disruption-type scenarios",  # just say "disruption scenarios"
    "the most likely path to episodic supply disruption rather than sustained shortage",
]
...
ECONOMIC_INDICATOR_FORMAT = {
    ...
    "examples": {
        ...
        "bad": [
            "Steel",  # no units
            "Industrial/Manufacturing",  # vague
        ],
    },
}
...
REPORT_STYLE_SYSTEM_MESSAGE = """You are generating content for an MRO Intelligence Report for Grainger.

CRITICAL STYLE REQUIREMENTS:

1. SECTION HEADERS
   - All major headers must be bold: **EXECUTIVE SUMMARY**
   - Subsection titles use Title Case: **Manufacturing Sector Outlook**
...
3. KEY FINDINGS
   ...
   - Include client transition: "Ergo assesses three implications for Grainger:"
   - Bullets use dash format with bold lead phrase + period:
     -   **Supply disruption.** Description here...

4. REMOVE INTERNAL NAMES
   - No scenario codenames or internal labels
...
7. ECONOMIC INDICATORS
   - Include units: "Steel Price (USD/ton)", "Crude Oil (USD/bbl)"
   - Focus on MRO demand drivers: manufacturing output, construction activity, energy prices
"""
...
FINAL_CHECKLIST = [
    ...
    "Economic indicators include units (USD/ton, USD/bbl)",
    "No semicolons within bullet points - use periods",
    "All content is directly relevant to MRO market demand and operations",
