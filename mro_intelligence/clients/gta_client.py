...
class GTAClient:
    ...
    async def get_sanctions_and_export_controls(
        ...
        """
        Get recent sanctions and export controls (critical for industrial)
    ...
    async def get_industrial_sector_interventions(
        self, days: int = 90, limit: int = 30
    ) -> List[GTAIntervention]:
        """
        Get interventions specifically affecting industrial and aerospace sectors
        ...
        Returns:
            List of industrial-related interventions
        """
        end_date = datetime.now()
        start_date = end_date - timedelta(days=days)

        request_data = {
            "affected_sectors": ["industrial", "aerospace", "equipment", "air transport"],
            "implementation_period": [
                start_date.strftime("%Y-%m-%d"),
                end_date.strftime("%Y-%m-%d"),
            ],
    ...
    async def get_immigration_visa_restrictions(
        ...
        Critical for MRO clients planning international travel.
