...
class ErgoMindConfig:
    ...
    def get_bearer_token(self) -> str:
        """
        Generate bearer token for ErgoMind API authentication.

        Per ErgoMind v2 API docs, the bearer token should be a base64-encoded JSON
        containing an 'email' field that matches the user_id.
        """
        token_data = {"email": self.user_id, "name": "MRO Intelligence"}
...
class ErgoMindClient:
    ...
    async def create_conversation(self, initial_message: str = "Hello") -> str:
        ...
        # ErgoMind API requires Bearer token - base64-encoded JSON with email
        headers = {
            "Authorization": f"Bearer {self.config.get_bearer_token()}",
    ...
    async def _execute_websocket_query(self, conversation_id: str, query: str) -> QueryResult:
        ...
        try:
            ...
            async with websockets.connect(
                ...
                logger.info("WebSocket connected with Bearer token")
